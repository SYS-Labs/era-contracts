...
export class Deployer {
  ...
  public async deployChainAdmin(create2Salt: string, ethTxOptions: ethers.providers.TransactionRequest) {
    ...
    const accessControlRestriction = await this.deployViaCreate2("AccessControlRestriction", [0, this.ownerAddress], create2Salt, ethTxOptions);
    if (this.verbose) {
    ...
    const contractAddress = await this.deployViaCreate2("ChainAdmin", [[accessControlRestriction]], create2Salt, ethTxOptions);
    if (this.verbose) {
